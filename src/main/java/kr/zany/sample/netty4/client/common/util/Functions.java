...
public class Functions {
    ...
    /**
     * <p><b>지정한 길이를 갖는 임의의 문자열을 만들어낸다.</b></p>
     *
     * @param len
     * @return
     */
    public static String makeRandString(int len) {
        return makeRandString(len, null);
    }

    /**
     * <p><b>지정한 길이를 갖는 임의의 문자열을 만들어낸다.</b></p>
     * <p>specialChars 값을 지정하면 해당 문자값을 포함하는 임의의 문자열을 만들어낸다. ex) "@#$%^&*!()" </p>
     *
     * @param len
     * @param specialChars
     * @return
     */
    public static String makeRandString(int len, String specialChars) {

        StringBuilder builder = new StringBuilder();

        final int typeSpecial = 0;
        final int typeLower   = 1;
        final int typeUpper   = 2;
        final int typeNumber  = 3;

        do {

            int charType = makeRandNumber(0,3);

            switch (charType) {
                case typeSpecial:
                    if (!StringUtils.isEmpty(specialChars)) {
                        builder.append(specialChars.charAt(makeRandNumber(0, specialChars.length()-1)));
                    }
                    break;
                case typeLower:  builder.append((char)(makeRandNumber((int)'a', (int)'z')));   break;
                case typeUpper:  builder.append((char)(makeRandNumber((int)'A', (int)'Z')));   break;
                case typeNumber: builder.append((char)(makeRandNumber((int)'0', (int)'9')));   break;
                default:
                    System.out.println("makeRandString ERROR.");
                    break;
            }
    ...
    public static String debugStringHashSet(Set<String> hashset) {
        return debugStringHashSet(hashset, false);
    }

    public static String debugStringHashSet(Set<String> hashset, boolean withList) {
        ...
        builder.append(String.format("%d item(s) found.", hashset.size()));
        return builder.toString();
    }

    public static void debugPrintHashSet(Set<String> hashset) {
        debugPrintHashSet(hashset, false);
    }

    public static void debugPrintHashSet(Set<String> hashset, boolean withList) {
        System.out.println(debugStringHashSet(hashset, withList));
    }
    ...
    public static Map<?,?> sortMapByKey(Map<?,?> unsortedMap) {
